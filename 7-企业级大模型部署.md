...
**学习建议：** 第 1 节先建立「应用层 + 推理层 + 选型」的全局图景，重点看懂 1.3 中**推理引擎 vs 托管平台**的辨析和**最终选型（Xinference）**；第 2 节按「租服务器 → Docker → Dify」顺序做，第 3 节按「租 GPU → Xinference → 部署三类模型 → Dify 插件对接」顺序做。遇到 Docker 基础、镜像网络、Dify 排障、升级、数据库连接问题可参考 [第 8 章](8-Docker入门与Dify部署常见问题.md)。
